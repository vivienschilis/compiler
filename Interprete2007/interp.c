...
DumpObject (object *obj) {
  ...
  fprintf (stderr, "obj#%d, address %d", obj->id, obj->fields);
  if (obj->name) fprintf (stderr, "\t%s", obj->name);
  fprintf (stderr, "\n");
   ...
   * l'interpreter !
   */
    fprintf (stderr, "\t[%02d] %d", i, *(obj->fields + i));
...
CopyStack (data *from, data *to, long len) {
  if (len < 0) Error ("nombre negatif de valeurs a copier", len);
  CheckStack (from, len);
  CheckStack (to, len);
  while (len--)	*to++ = *from++;
}
...
Run () {
  long	i, n; char *s; long *addr; char readbuf [2048];
...
Error(char *msg, void *a1, void *a2) {
  char buf[1024];
  data *oldsp;

  sprintf (buf, "### erreur : %s\n", msg);
  fprintf (stderr, buf, a1, a2);
  DumpInstr (pc);
